...
# =========================================================
# 7. 마크다운 깨짐 방지 텍스트 정리 함수 (신규 추가)
# =========================================================
def sanitize_markdown(text: str) -> str:
    """
    Streamlit markdown 렌더링에서 깨질 수 있는 특수문자를 정리한다.
    특히 문장 끝에 단독으로 남은 마크다운 기호들을 제거한다.
    """
    ...
    # 문장 끝에 단독으로 남아있는 마크다운 기호 제거
    dangerous_endings = ("*", "_", "`")

    while text.endswith(dangerous_endings):
        text = text[:-1].rstrip()
...
for msg in st.session_state.messages:
    ...
    with st.chat_message(msg["role"], avatar=role_icon):
        # 출력 전에 마크다운 정리 적용
        safe_text = sanitize_markdown(msg["content"])
